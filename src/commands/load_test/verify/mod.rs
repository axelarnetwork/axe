...
/// Verify EVM/Solana/Stellar → Sui ITS transactions. Mirrors
/// `verify_onchain_xrpl_its` but uses Sui's `MessageApproved` /
/// `MessageExecuted` events on the AxelarGateway package to detect
/// destination-side approval and execution. `gateway_pkg` is the Sui
/// AxelarGateway Move-package address.
///
/// Currently unused: ships ahead of a `*_to_sui` ITS runner so wiring one
/// up is just a dispatcher change.
    ...
    let peaks = run_its_hub_pipeline(
        ...
        RunItsHubArgs {
            ...
            dest: ItsHubDest::Sui {
                ...
                gateway_pkg: gateway_pkg.to_string(),
            },
        },
    )
    .await?;
...
/// Streaming variant of `verify_onchain_sui_its` — runs concurrently with
/// the send phase, receiving confirmed txs via the channel.
#[allow(dead_code, clippy::too_many_arguments)]
pub async fn verify_onchain_sui_its_streaming(
    config: &Path,
    source_chain: &str,
    destination_chain: &str,
    sui_rpc: &str,
