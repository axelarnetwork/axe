...
// ---------------------------------------------------------------------------
// Polish-side helpers (re-introduced after the polish→main merge).
// These were previously split into `solana/balance.rs` + `solana/rpc.rs`
// on the polish branch; they're back here as plain top-level fns until
// Phase 5 redoes the directory split on the merged tree.
// ---------------------------------------------------------------------------
...
pub fn check_solana_balance(
    ...
    if balance < min_lamports {
        crate::ui::error(&format!("{label} Solana wallet underfunded:"));
        crate::ui::error(&format!("  address: {pubkey}"));
        crate::ui::error(&format!("  rpc:     {rpc_url}"));
        crate::ui::error(&format!(
            "  balance: {display:.6} SOL (need >= {min_display:.6})"
        ));
        if balance == 0 {
            crate::ui::error("  account has zero SOL — fund or airdrop before retrying");
            crate::ui::error(&format!("    solana airdrop 2 {pubkey} --url {rpc_url}"));
        }
        return Err(eyre::eyre!(
