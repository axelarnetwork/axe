...
The matrices below show which protocols (`GMP`, `ITS`, or both) axe dispatches
for each (source → destination) chain pair, by network. Cells where forward and
reverse directions differ are marked with `*` and detailed below the table.

`GMP+ITS` means both protocols are wired. `GMP` or `ITS` alone means only that
protocol is wired. `—` means neither.
...
| `axe propose`        | Submit & (optionally) relay an ASG governance proposal | self |
